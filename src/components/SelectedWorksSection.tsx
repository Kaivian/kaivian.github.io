  ...
  return (
    <section id="work" className="w-full bg-[#F4F1EA] text-[#1A1A1A] font-libre-franklin py-12 border-b-4 border-[#1A1A1A]">
      <div className="max-w-275 mx-auto px-4 md:px-0">
        ...
        {/* Grid for Exhibits B through G (3-Column Grid with Staggered Reveal) */}
        <ScrollStaggerGroup staggerDelay={0.12} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-10">
          {gridWorks.map((work) => (
            <ScrollStaggerItem key={work.exhibit} direction="up" distance={30}>
              <div
                className="flex flex-col justify-between border-b md:border-b-0 border-[#1A1A1A]/30 pb-8 md:pb-0 group cursor-pointer h-full"
